...
Before including the imperfect detection aspect, let's see the general dynamic of abundance and acoustic detections during the morning of sampling
```{r}
...
ggsave(filename = "figs/abundance-by-time-continuous.jpg", width = 12, height = 7, units = "in", dpi = 300)
...
data |> 
  ...
  geom_point(alpha = 0.6) + 
  facet_wrap(~common_name, ncol = 4, scale = "free_y") + 
  scale_color_manual(values=met.brewer("Homer2", 4))+
  labs(x = "Time",
  ...
  theme_minimal()+
  theme(legend.position = "bottom")
ggsave(filename = "figs/acu_detections-by-time-continuous.jpg", width = 12, height = 7, units = "in", dpi = 300)
...
```{r}
# create bins for time_of_day
data <- data %>%
  mutate(
    hour = as.numeric(substr(start_time, 1, 2)),
    time_of_day = case_when(
      hour >= 5 & hour < 6 ~ "5AM to 6AM",
      hour >= 6 & hour < 7 ~ "6AM to 7AM",
      hour >= 7 & hour < 8 ~ "7AM to 8AM", 
      hour >= 8 & hour < 9 ~ "8AM to 9AM",
      hour >= 9 & hour < 10 ~ "9AM to 10AM",
      TRUE ~ "Other"
    )
  ) %>%
  dplyr::select(-hour)

# create abundance by time of day 
fig_abundance_by_time <- data%>%
  ggbetweenstats(x = time_of_day,
                         y = abundance,
                         xlab = "Time of Day",
                         ylab = "Counts of Individuals (Abundance)", 
...
# save
ggsave(fig_abundance_by_time, filename = "figs/abundance-by-time-of-day.png", width = 12, height = 7, device = png(), units = "in", dpi = 300)
dev.off()

# we notice that besides 9am to 10am where there are fewer point counts & acoustic data, there aren't any differences in abundance across times

# detections by time of day 
fig_detections_by_time <- data%>%
  ggbetweenstats(x = time_of_day,
                         y = detections,
                         xlab = "Time of Day",
...
# when pooled across species and examining differences in detections by time of day, we see no differences. 

# save
ggsave(fig_detections_by_time, filename = "figs/detections-by-time-of-day.png", width = 12, height = 7, device = png(), units = "in", dpi = 300)
dev.off()
```

## Are there differences in abundances/detections within species across times of day?

While no differences were observed when datasets were pooled, we test if there are differences within species across times of day.
```{r}
# abundance and detections side by side
plots <- list()
...
for(i in 1:length(unique(data$common_name))){
  ...
  # abundance plot
  abundance_plot <- ggplot(for_plot, 
         aes(x = time_of_day, 
             y = abundance, 
             fill = factor(time_of_day))) +
    geom_boxplot(alpha = 0.7, width = 0.2, size = 0.3) +  
    scale_fill_manual(
      values = met.brewer("Tiepolo",5),
      name = "Time of Day"
    ) +
    theme_bw() +
    labs(
  ...
  # detections plot
  detections_plot <- ggplot(for_plot, 
         aes(x = time_of_day, 
             y = detections, 
             fill = factor(time_of_day))) +
    geom_boxplot(alpha = 0.7, width = 0.2, size = 0.3) +  
  ...
  combined_plot <- abundance_plot + detections_plot + 
...
cairo_pdf(
  filename = "figs/abundance-detections-by-species-time-of-day.pdf",
  width = 16, height = 8,  
  onefile = TRUE
)
